...
// yarn run ts-node --files .\scripts\Delegation.ts <tokenContractAddress> <delegatedAddress>
...
async function main() {
  ...
  if (params.length != 2)
    throw new Error(
      `Expected 2 arguments for tokenContractAddress and delegatedAddress but received ${params.length} argument(s) instead.`
    );
  ...
  console.log(
    `Prior to delegating, the address ${delegatedAddress} has ${votesPrior} votes.`
  );
  ...
  console.log(
    `After delegating, the address ${delegatedAddress} has ${votesAfter} votes.`
  );
}
